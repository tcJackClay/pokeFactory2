...
- 事件奖励、加入队伍、局内奖励等弹窗中的核心展示素材（道具图标、宝可梦 sprite、特殊奖励主体图）默认直接放在弹窗主体内展示，不再额外包裹一层独立外框、小卡片描边或装饰底座。
- 需要强化奖励感时，优先使用尺寸、留白、投影、文案层级来突出主体，不再使用“弹窗里再套一个奖励框”的做法。
- 宝可梦类奖励展示允许比道具图标更大，优先保证主体辨识度和角色存在感，避免被内层装饰框挤压。
- 后续新页面或重构旧页面时，凡是复用奖励弹窗样式，统一遵守本规则。
